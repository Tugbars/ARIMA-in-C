...
double *performMultivariateLinearRegression(int numObservations, int numPredictors, double X[][numPredictors], double Y[][1])
{
  ...
  for (int i = 0; i < numPredictors; i++)
    XtX[i][i] += lambda; // XtX[i][i] = XtX[i][i] + λ
  ...
  else if (n == 2)
  {
    ...
    double det = a * d - b * c;
    if (fabs(det) < 1e-12)
      det += lambda; // Adjust determinant if near-zero
    XtX_inv[0][0] = d / det;
    XtX_inv[0][1] = -b / det;
    XtX_inv[1][0] = -c / det;
    XtX_inv[1][1] = a / det;
  ...
  for (int i = 0; i < numPredictors; i++)
    estimates[i] = beta[i][0]; // Store betas
...
double adfPValue(double tstat, int modelType)
{
  ...
  if (tstat <= cv[0])
    return 0.005; // Below 1% critical value
  if (tstat >= cv[2])
    return 0.15; // Above 10% critical value
...
int computeCompanionEigenvalues(double coeffs[], int order, double complex *roots)
{
  ...
  for (int j = 0; j < order; j++)
    A[j] = -coeffs[j]; // Top row: -φ_j for AR convention
  for (int i = 1; i < order; i++)
    A[i * order + (i - 1)] = 1.0; // Subdiagonal 1s
  ...
  for (int iter = 0; iter < 10; iter++)
  {
    ...
    for (int i = 0; i < order; i++)
    {
      for (int j = i; j < order; j++)
        R[i * order + j] = tempA[i * order + j]; // Upper triangle
    }
    double *Q = calloc(order * order, sizeof(double));
    for (int i = 0; i < order; i++)
      Q[i * order + i] = 1.0; // Initialize Q as identity
    double *b = malloc(order * sizeof(double));
    for (int k = 0; k < order; k++)
    { // Build Q column-wise using Householder reflectors
      for (int i = 0; i < order; i++)
        b[i] = (i == k) ? 1.0 : 0.0;           // Unit vector e_k
      applyQTranspose(order, tempA, order, b); // Q^T e_k
      for (int i = 0; i < order; i++)
        Q[i * order + k] = b[i]; // Column k of Q
    }
...
double computeMANegLogLikelihood(double theta[], double residuals[], int n, int q)
{
  ...
  for (int t = q; t < n; t++)
  {
    ...
    if (abs_err <= delta)
      sum_loss += 0.5 * abs_err * abs_err; // Quadratic: \( \frac{1}{2} \epsilon^2 \)
    else
      sum_loss += delta * (abs_err - 0.5 * delta); // Linear: \( \delta (|\epsilon| - \frac{\delta}{2}) \)
  }
...
double computeARNegLogLikelihood(double phi[], double series[], int n, int p)
{
  ...
  for (int t = p; t < n; t++)
  {
    ...
    for (int j = 0; j < p; j++)
      pred += phi[j] * series[t - j - 1]; // Add AR terms: Σφ_j y_{t-j}
...
void estimateARWithCMLE(double series[], int length, int p, double phi[])
{
  ...
  if (!theta)
  {
    fprintf(stderr, "Memory allocation error in estimateARWithCMLE.\n");
    exit(EXIT_FAILURE);
  }
  ...
  for (int iter = 0; iter < MAX_NEWTON_ITER; iter++)
  {
    ...
    for (int t = p; t < length; t++)
    {
      ...
      for (int j = 0; j < p; j++)
        pred += theta[j] * series[t - j - 1]; // Add AR terms: Σφ_j y_{t-j}
      errors[t] = series[t] - pred;           // ε_t = y_t - (μ + Σφ_j y_{t-j})
      for (int i = 0; i < p; i++)
      {
        grad[i] += -errors[t] * series[t - i - 1]; // ∂(-logL)/∂φ_i = -Σε_t y_{t-i-1}
        for (int j = 0; j <= i; j++)
          hess[i][j] += series[t - i - 1] * series[t - j - 1]; // ∂²(-logL)/∂φ_i∂φ_j ≈ Σy_{t-i-1} y_{t-j-1}
      }
    ...
    for (int i = 0; i <= p; i++)
      new_theta[i] = theta[i] - step * delta[i]; // θ_new = θ - step * Δθ
    ...
    while (new_nll > old_nll && step > NEWTON_TOL)
    {
      ...
      for (int i = 0; i <= p; i++)
        new_theta[i] = theta[i] - step * delta[i];
    ...
    free(errors);
    if (step < NEWTON_TOL)
      break; // Converged if step size too small
  }

  // Step 8: Copy final estimates to output
  for (int i = 0; i < p; i++)
    phi[i] = theta[i]; // AR coefficients
...
void hannanRissanenMAInit(double residuals[], int length, int q, double theta[])
{
  ...
  if (!ar_temp)
  {
    fprintf(stderr, "Memory allocation error in hannanRissanenMAInit.\n");
    exit(EXIT_FAILURE);
  }
  ...
  for (int t = p_temp; t < length; t++)
  {
    ...
    for (int j = 0; j < p_temp; j++)
      pred += ar_temp[j] * residuals[t - j - 1]; // y_t = Σφ_j y_{t-j}
  ...
  for (int t = p_temp; t < length; t++)
  {
    ...
    for (int j = 0; j < q; j++)
      X[t - p_temp][j] = (t - j - 1 >= p_temp) ? ar_resid[t - p_temp - j - 1] : 0.0; // X[t][j] = ε_{t-j-1}
  }
  ...
  for (int i = 0; i < q; i++)
    theta[i] = ma_est[i]; // MA coefficients θ_j
...
double *forecastARIMA(double series[], int seriesLength, int p, int d, int q)
{
  ...
  if (p > 0 && currentLength > p)
  {
    ...
    for (int i = 0; i < p; i++)
      DEBUG_PRINT("phi[%d]=%.4f, ", i, arEstimates[i]);
    DEBUG_PRINT("mu=%.4f\n", arEstimates[p]);
  ...
  if (q > 0 && arResiduals && currentLength - p > q)
  {
    ...
    for (int i = 0; i < q; i++)
      DEBUG_PRINT("theta[%d]=%.4f, ", i, maEstimates[i]);
    DEBUG_PRINT("mu=%.4f\n", maEstimates[q]);
  }
  else
  {
  ...
  {
    for (int j = 0; j < q; j++)
      oneStep += maEstimates[j] * pastErrors[j];
  ...
  for (int i = 0; i < p; i++)
    psi[i] = arEstimates ? arEstimates[i] : 0.0;
  for (int i = 0; i < q; i++)
    psi[p + i] = maEstimates ? maEstimates[i] : 0.0;
  double sigma2 = 0.0;
  if (arResiduals)
  {
    double sum_sq = 0.0;
    ...
    for (int i = 0; i < n; i++)
      sum_sq += arResiduals[i] * arResiduals[i];
    sigma2 = sum_sq / n;
  ...
  for (int h = 1; h < FORECAST_HORIZON; h++)
  {
    for (int j = 0; j < h && j < p + q; j++)
      var += sigma2 * psi[j] * psi[j];
...
double computeResidualACF(const double residuals[], int length, int maxLag, double acf[])
{
  ...
  {
    if (fabs(acf[i]) > max_acf)
      max_acf = fabs(acf[i]); // \( max(|r_k|) \) for k > 0
  }
  return max_acf;
}
...
void computeDiagnostics(double *series, int length, double *forecast, int p, int d, int q)
{
  ...
  for (int t = p; t < adjLength; t++)
  {
    ...
    for (int j = 0; j < p; j++)
      pred += arEst[j] * current[t - j - 1]; // Add AR terms
  ...
  for (int t = q; t < adjLength; t++)
  {
    ...
    for (int j = 0; j < q && t - j - 1 >= p; j++)
      pred += maEst[j] * arRes[t - j - 1 - p]; // Add MA terms
